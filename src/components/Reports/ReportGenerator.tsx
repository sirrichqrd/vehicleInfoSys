...
interface ReportGeneratorProps {
  ...
  vehicles: Vehicle[];
  logs: DriverLog[];
  onGenerateReport?: () => void; // ✅ optional callback
}
...
export function ReportGenerator({
  ...
  const filteredLogs = logs.filter((log) => {
    const vehicleMatch =
      selectedVehicles.length === 0 || selectedVehicles.includes(log.vehicleId);
    const driverMatch =
      selectedDrivers.length === 0 || selectedDrivers.includes(log.driverId);
    const typeMatch =
      selectedLogTypes.length === 0 || selectedLogTypes.includes(log.type);
  ...
  const handleLogTypeToggle = (logType: string, checked: boolean) => {
    setSelectedLogTypes((prev) =>
      checked ? [...prev, logType] : prev.filter((t) => t !== logType)
    );
  };

  const generateReport = () => {
    setShowReport(true);
    onGenerateReport?.(); // ✅ trigger activity logging
  };
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Generate comprehensive vehicle and driver reports
          </p>
        </div>
      </div>
      ...
      {!showReport ? (
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div>
              <Label className="text-base font-medium">
                Select Drivers (leave empty for all)
              </Label>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 mt-2">
                {drivers.map((driver) => (
                ...
                {["remittance", "maintenance", "expense", "incident"].map((type) => (
                  <div key={type} className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        handleLogTypeToggle(type, checked as boolean)
                      }
                    />
            ...
            <CardHeader>
              <div className="flex justify-between items-center">
                ...
                <div className="flex space-x-2">
                  ...
                  <Button onClick={() => setShowReport(false)}>New Report</Button>
                </div>
              </div>
            </CardHeader>
            <CardContent>
